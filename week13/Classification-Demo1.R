  ...
  DT_mod <- ctree(Donate ~ ., data=Blood_train)
  ...
  # visualize the model:
  plot(DT_mod, gp=gpar(fontsize=8))
  
  # predict X_test using the trained model:
  pred_DT <- predict(DT_mod, newdata=X_test)
  
  CF_DT <- table(Actual=Y_test_gt, Predicted=pred_DT)
  
  CF_DT
  Accu_test_DT <- (CF_DT[2,2] + CF_DT[1,1])/sum(CF_DT)
  round(Accu_test_DT, 4)
